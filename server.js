...
// Language access by plan tier (security layer - frontend can be bypassed)
const LANGUAGE_ACCESS = {
  free: ['English', 'Urdu', 'Arabic'],
  regular: ['English', 'Urdu', 'Arabic', 'French', 'Spanish', 'German', 'Chinese', 'Hindi'],
  // pro and above: all languages allowed (no restriction)
};

function isLanguageAllowedForPlan(plan, lang) {
  if (!lang) return true;
  if (plan === 'free') return LANGUAGE_ACCESS.free.includes(lang);
  if (plan === 'regular') return LANGUAGE_ACCESS.regular.includes(lang);
  return true; // pro, business, agency_starter, agency_pro → all allowed
}
...
const LENGTH_CONFIG = {
  auto: {
    maxTokens: 400,
    words: '70-130 words (varies by email type)',
    instruction: 'Write the optimal length for this email type. Cold outreach: 80-120 words. Proposal: 100-160 words. Follow-up: 50-80 words. Reply: match their email length. Apology: 80-120 words. Always cut padding.',
  },
  short: {
    maxTokens: 130,
    words: '40-60 words',
    instruction: 'CRITICAL: 40-60 words MAX for the body. 2-4 short sentences. NO multiple paragraphs. Get to the point in the FIRST sentence. Every word must earn its place.',
  },
  medium: {
    maxTokens: 240,
    words: '80-130 words',
    instruction: 'STRICT: 80-130 words body. 2 paragraphs MAX (NOT 3). Say each point exactly ONCE. Clean structure with one paragraph break.',
  },
  detailed: {
    maxTokens: 550,
    words: '180-280 words',
    instruction: 'LENGTH: 180-280 words, 3 paragraphs MAX. Every paragraph must say something NEW. Suitable for complex proposals or detailed explanations.',
  },
};

const TONE_GUIDE = {
  Professional: 'Polished and direct. Use "Hi [Name]," as greeting (not "Dear"). Respectful but not stiff. Confident without arrogance. Skip pleasantries, get to value fast.',
  Formal:       'Traditional business phrasing. "Dear [Name]," and "Sincerely,". Full words, no contractions. Use for legal, executive, or first-contact-with-senior contexts.',
  Friendly:     'Warm and conversational. "Hi [Name]," or "Hey [Name],". Contractions (I\'m, you\'re, we\'ll). Like writing to a colleague you know. Can use occasional humor.',
  Persuasive:   'Confident and benefit-focused. Lead with specific outcome or number, not features. Use proof points (numbers, examples, names). Close with low-friction CTA. Subtle urgency, never pushy.',
  Casual:       'Relaxed and human. "Hey [Name]," greeting. Short sentences, fragments OK. Contractions everywhere. Like texting a colleague. Skip formal sign-offs.',
  Empathetic:   'Acknowledge the situation or feeling first. Soft, human, specific. No platitudes ("I understand how you feel"). Validate, then help. Use for sensitive topics, apologies, support.',
};

// Email type-specific frameworks - tells AI HOW to structure each email type
const TYPE_FRAMEWORKS = {
  'Cold outreach': `COLD OUTREACH STRUCTURE:
1. Subject: Curiosity or specificity. Examples: "quick question, [Name]" / "[specific number] in [timeframe]?" / "saw your [specific thing]"
2. Hook: ONE specific personalized observation about recipient (their post, hire, launch, work)
3. Reason: Why you're reaching out (1 short sentence)
4. Value: What you offer + concrete proof (specific numbers, real examples)
5. CTA: Soft low-friction ask (15-min call, free resource, simple yes/no)
RULE: First line must show you researched them. No "I came across your company"`,

  'Cold application': `COLD JOB APPLICATION STRUCTURE:
1. Subject: Role + brief differentiator
2. Hook: Why this company specifically (show research)
3. Credentials: Most relevant 1-2 achievements with numbers
4. Fit: How your experience maps to their needs
5. CTA: Specific next step (call, portfolio, interview)`,

  'Business proposal': `BUSINESS PROPOSAL STRUCTURE:
1. Subject: Clear outcome or value
2. Brief context (what we discussed/their need)
3. Proposed scope: 3-4 specific deliverables (use bullets if multiple)
4. Investment + timeline (transparent, no vague "competitive pricing")
5. Clear next step
RULE: Be scannable. Numbers and bullets beat walls of text.`,

  'Follow-up': `FOLLOW-UP STRUCTURE:
1. Subject: Reference original without sounding desperate ("bumping this", "following up")
2. Brief context (1 line max)
3. Add NEW value (insight, tip, case study, example) - DON'T just re-pitch
4. Easy yes/no question
RULE: Keep it shorter than original email. Long follow-ups feel pushy.`,

  'Reply': `REPLY STRUCTURE:
1. Address each specific point/question they raised
2. Match their energy level and formality
3. Provide clear answers or next steps
4. End with appropriate CTA based on their email's intent
RULE: No generic "thanks for reaching out". Match length to their message.`,

  'Introduction': `INTRODUCTION STRUCTURE:
1. Warm greeting
2. Who you are + relevant credibility (1 line)
3. Why this connection matters (specific reason)
4. Clear ask or offer
5. Make it easy to respond`,

  'Apology': `APOLOGY STRUCTURE:
1. Direct acknowledgment of issue (no excuses upfront, no "but")
2. Brief explanation of what happened (don't blame-shift)
3. Specific fix or compensation (concrete action)
4. Prevention going forward (what changes)
5. Clear next step to make it right
RULE: Take ownership. "I'm sorry" beats "We apologize for any inconvenience"`,

  'Invoice': `INVOICE/PAYMENT STRUCTURE:
1. Subject: Clear and specific (Invoice #, amount, status)
2. Polite but direct opening
3. State facts: invoice number, amount, original due date
4. Ask for specific action (payment date confirmation)
5. Maintain relationship - no threats or guilt
RULE: Professional, not emotional. Stick to facts and dates.`,

  'Update': `STATUS UPDATE STRUCTURE:
1. Brief status headline (1 line)
2. What's done (concrete progress)
3. What's next (specific milestones)
4. Any blockers or asks
5. Clear next checkpoint
RULE: Lead with the most important info. No buried updates.`,

  'Negotiation': `NEGOTIATION STRUCTURE:
1. Acknowledge their position briefly
2. Your counter with clear reasoning (numbers, examples)
3. What you're offering in return
4. Specific proposal
5. Open the door for discussion
RULE: Confident but flexible. Show you understand their constraints.`,

  'Reminder': `REMINDER STRUCTURE:
1. Subject: Specific reminder ("Friday deadline", "Tomorrow's call")
2. Brief, friendly reminder
3. Key details (date, time, what's needed)
4. Easy confirmation ask
RULE: Keep it short. Reminders should feel helpful, not nagging.`,
};
...
app.post('/api/generate', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Enforce language access by plan (security layer)
    const requestedLang = lang || 'English';
    if (!isLanguageAllowedForPlan(user.plan, requestedLang)) {
      const upgradeMsg = (user.plan === 'free')
        ? `${requestedLang} requires Regular plan ($6.95/mo) or Pro ($11.95/mo) for all 30+ languages. Please upgrade.`
        : `${requestedLang} requires Pro plan ($11.95/mo) or higher for all 30+ languages. Please upgrade.`;
      return res.status(403).json({ error: upgradeMsg, code: 'LANGUAGE_LOCKED' });
    }
    ...
    // Pick framework based on email type (NEW)
    const typeFramework = TYPE_FRAMEWORKS[emailType] || '';

    // Smart temperature based on email type (NEW)
    // Cold/Persuasive needs creativity for hooks. Formal/Invoice needs consistency.
    let temperature = 0.7; // default
    if (selectedTone === 'Persuasive' || emailType === 'Cold outreach') temperature = 0.85;
    else if (selectedTone === 'Casual' || selectedTone === 'Friendly') temperature = 0.8;
    else if (selectedTone === 'Formal' || emailType === 'Invoice' || emailType === 'Apology') temperature = 0.4;
    else if (emailType === 'Follow-up' || emailType === 'Reminder') temperature = 0.5;
...
═══════════════════════════════════════════════
${typeFramework || 'Use clear opening, focused middle with specific points, strong CTA.'}
...
CRITICAL: The tone determines BOTH greeting and sign-off. They must MATCH the tone.
- Professional → "Hi [Name]," + "Best," or "Thanks,"
- Formal → "Dear [Name]," + "Sincerely," or "Regards,"
- Friendly → "Hi [Name]," or "Hey [Name],"  + "Thanks," or "Cheers,"
- Persuasive → "Hi [Name]," + "Best," or "Thanks,"
- Casual → "Hey [Name]," + "Thanks," or just "—Name"
- Empathetic → "Hi [Name]," + "Take care," or "Thinking of you,"

NEVER use "Dear" with Friendly/Casual/Persuasive tones.
NEVER use "Sincerely" with Friendly/Casual/Persuasive tones.
"Sincerely" and "Dear" are ONLY for Formal tone.

═══════════════════════════════════════════════
LANGUAGE: ${selectedLang}
═══════════════════════════════════════════════
Write the ENTIRE email (subject + greeting + body + sign-off + name) in ${selectedLang}.

🔴 CRITICAL LANGUAGE RULES:

1. **NO code-mixing.** Do NOT mix English words into ${selectedLang} text. Translate everything.
   ❌ WRONG: "Aap ki email marketing agency ka LinkedIn par kaam dekha"
   ✅ RIGHT (Urdu): "آپ کی ای میل مارکیٹنگ ایجنسی کا کام لنکڈ ان پر دیکھا"
   
   EXCEPTION: Only proper nouns (brand names like LinkedIn, Gmail, MailCraft, ChatGPT) and 
   currency ($99, €50) stay in original form. Everything else translates.

2. **GREETING must be native ${selectedLang}.** 
   ❌ WRONG: "Hi Mr. Imran" (English greeting in Urdu email)
   ✅ For Urdu: "السلام علیکم محترم عمران صاحب،" or "محترم عمران صاحب،"
   ✅ For Arabic: "السلام عليكم السيد عمران،"
   ✅ For Spanish: "Estimado Sr. Imran,"
   ✅ For French: "Cher M. Imran,"
   ✅ For German: "Sehr geehrter Herr Imran,"
   
3. **SIGN-OFF must be native ${selectedLang}.** ABSOLUTELY NEVER use English sign-offs 
   (Thanks, Best, Regards, Cheers) in non-English emails.
   
   For URDU professional emails use: "شکریہ،" / "والسلام،" / "مخلص،" / "آپ کا مخلص،"
   For URDU friendly emails use: "شکریہ،" / "خیرخواہ،"
   For URDU formal/empathetic use: "آپ کا مخلص،" / "نیک خواہشات کے ساتھ،"
   
   For ARABIC use: "شكراً،" / "مع التحية،" / "وتقبلوا فائق الاحترام،"
   For SPANISH use: "Saludos," / "Atentamente," / "Cordialmente,"
   For FRENCH use: "Cordialement," / "Bien à vous," / "Salutations,"
   For GERMAN use: "Mit freundlichen Grüßen," / "Beste Grüße,"
   For HINDI use: "धन्यवाद," / "सादर," / "शुभकामनाएं,"
   For other languages: use the natural professional sign-off in that language. NEVER default to English.

4. **NAME at end:** Write the user's name in their native script if appropriate, 
   OR keep their original name (e.g., "Najeeb Sultan" works in any language). 
   For Urdu/Arabic emails, transliterating to native script is preferred but optional.

5. **PUNCTUATION:** Use ONLY the punctuation native to ${selectedLang}.
   - Urdu uses: ۔ (period), ، (comma), ؟ (question mark)
   - Arabic uses: ، (comma), ؟ (question mark)
   - Do NOT mix with English . , ?
   ❌ WRONG: "میں۔." (mixing Urdu and English period)
   ✅ RIGHT: "میں۔" (only Urdu period)

6. **COMPLETENESS:** ALWAYS finish the email properly with:
   - Full body covering all key points
   - Proper sign-off in ${selectedLang}
   - User's name on the next line
   Never stop mid-sentence. Never skip the closing.
...
- "I'm writing to..." (ANY variant — banned in all forms including "I'm writing to own", "I'm writing to inform", etc. Just start with the action directly: "Owning the delay" not "I'm writing to own the delay")
...
✓ "Hope you're doing well" (ONLY for cold outreach, introduction — NEVER for apology, invoice, follow-up, application, thank you)
✓ "Hope your week's going well" (ONLY for cold outreach)
✓ "Hope you're well" (ONLY friendly tone, ONLY first contact)
✓ "Quick one —" / "Quick update —"
✓ "Wanted to flag —"
✓ "Quick thought on X —"
✓ "Noticed X, thought you'd want to know"

CRITICAL: For apologies, invoices, follow-ups, job applications, and thank you emails — SKIP "Hope you're doing well" entirely. Get straight to the point. These email types should never start with pleasantries.
...
Count words. Is body within ${lengthCfg.words}? If over, cut. Length is a HARD rule.${voiceBlock}`;
    ...
    // Language token multiplier: Non-Latin scripts use 2-3x more tokens per character/word
    // than English. Without this, "Short & concise" (130 tokens) gets cut mid-sentence in Urdu.
    const NON_LATIN_LANGUAGES = [
      'Urdu', 'Arabic', 'Hindi', 'Bengali', 'Tamil', 'Punjabi', 'Persian (Farsi)',
      'Hebrew', 'Chinese', 'Japanese', 'Korean', 'Russian', 'Ukrainian', 'Greek',
      'Thai', 'Vietnamese' // Vietnamese uses Latin but with many diacritics → more tokens
    ];
    const isNonLatinScript = NON_LATIN_LANGUAGES.includes(selectedLang);
    const languageMultiplier = isNonLatinScript ? 2.5 : 1.0;
    ...
    // Language multiplier applied on top to handle script density differences.
    const baseBudget = useMyVoice
    ...
    const tokenBudget = Math.round(baseBudget * languageMultiplier);

    // Call AI with retry logic (1 retry on transient errors)
    let response;
    let lastErr;
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        lastErr = apiErr;
        // Only retry on 429 (rate limit) or 5xx (server errors), not on 401/400
        const shouldRetry = apiErr.status === 429 || (apiErr.status >= 500 && apiErr.status < 600);
        if (!shouldRetry || attempt === 1) throw apiErr;
        console.warn(`AI call attempt ${attempt + 1} failed (${apiErr.status}), retrying...`);
    ...
    // Safety net: ensure email is complete (has sign-off + name) — language-aware
    email = ensureCompleteEmail(email, req.user.name || 'there', selectedLang);
...
function ensureCompleteEmail(email, userName, language) {
  if (!email || typeof email !== 'string') return email;

  const trimmed = email.trim();
  const isEnglish = !language || language === 'English';

  // Already has English sign-off + name? (allows multi-word names and trailing period)
  const hasValidEnding = /\n\s*(Thanks|Cheers|Best|Regards|Sincerely|Yours|Take care|Talk soon|Best regards|Kind regards|Warm regards)[\s,]*\n\s*\S+(?:\s+\S+)*\s*[.!]?\s*$/i.test(trimmed);
  if (hasValidEnding) return trimmed;

  // For non-English languages: detect ANY common closing pattern in last few lines
  // We trust that AI gave a sign-off in the target language and don't append English fallback
  if (!isEnglish) {
    const lines = trimmed.split('\n').filter(l => l.trim());
    const lastFewLines = lines.slice(-3).join('\n');
    
    // Multi-language sign-off patterns (common closings across languages)
    const multiLangSignOffPatterns = [
      /شکریہ|والسلام|مخلص|نیک خواہشات|آپ کا/,           // Urdu
      /شكرا|تحياتي|مع التحية|تقبلوا/,                    // Arabic
      /Saludos|Atentamente|Cordialmente|Un saludo/i,    // Spanish
      /Cordialement|Bien à vous|Salutations|Sincèrement/i, // French
      /freundlichen Grüßen|Beste Grüße|Viele Grüße|Herzliche/i, // German
      /धन्यवाद|सादर|शुभकामनाएं/,                          // Hindi
      /Üdvözlettel|Tisztelettel|Köszönöm/i,             // Hungarian
      /Saudações|Atenciosamente|Cumprimentos/i,         // Portuguese
      /С уважением|Спасибо|С наилучшими/i,             // Russian
      /谢谢|此致|敬礼|此致敬礼/,                           // Chinese
      /ありがとう|よろしく|敬具/,                          // Japanese
      /감사합니다|안녕히|드림/,                            // Korean
    ];
    
    const hasNativeSignOff = multiLangSignOffPatterns.some(p => p.test(lastFewLines));
    if (hasNativeSignOff) return trimmed; // AI gave proper native sign-off, return as-is
    
    // For non-English with no detectable sign-off: still trust AI output rather than 
    // appending broken English. Just append the user's name on a new line if missing.
    const lastLine = lines[lines.length - 1] || '';
    if (lastLine.toLowerCase().includes(userName.toLowerCase().split(' ')[0])) {
      return trimmed; // Name already present
    }
    return trimmed + '\n' + userName;
  }

  // ENGLISH-only logic below
  ...
  // Proper sentence end but no sign-off → append (English only)
  return trimmed + '\n\nThanks,\n' + userName;
}
...
const LEMON_CONFIG = {
  ...
  variants: {
    ...
    agency_starter: process.env.LEMON_VARIANT_AGENCY_STARTER || '',
    agency_pro: process.env.LEMON_VARIANT_AGENCY_PRO || '',
  },
};
...
function getPlanFromVariant(variantId) {
  ...
  if (v === LEMON_CONFIG.variants.agency_starter) return 'agency_starter';
  if (v === LEMON_CONFIG.variants.agency_pro) return 'agency_pro';
  return null;
}
